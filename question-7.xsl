...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    <xsl:template match="/regularite-tgv">
        ...
        <xsl:variable name="minAnnee">
            <xsl:for-each select="//mesure/@annee">
        ...
        <xsl:variable name="minMois">
            <xsl:for-each select="//mesure[@annee=$minAnnee]/@mois">
                <xsl:sort data-type="number" order="ascending"/>
                <xsl:if test="position() = 1">
                    <xsl:value-of select="."/>
                </xsl:if>
        ...
        <xsl:variable name="maxMois">
            <xsl:for-each select="//mesure[@annee=$maxAnnee]/@mois">
                <xsl:sort data-type="number" order="descending"/>
                <xsl:if test="position() = 1">
                    <xsl:value-of select="."/>
                </xsl:if>
        ...
        <html>
            <head>
                <link rel="stylesheet" type="text/css" href="question-7.css" />
            </head>
            <body>
                <h1>
                    Régularité des TGV entre <xsl:value-of select="$minMois" />/<xsl:value-of select="$minAnnee" /> et <xsl:value-of select="$maxMois" />/<xsl:value-of select="$maxAnnee" />
                </h1>
                <xsl:for-each
                    select="axe">
                    <div class="axe">
